"""add created_at to knowledge_injection_job

Revision ID: 2802eb62c668
Revises: add_security_columns_20260902
Create Date: 2026-09-02 18:05:52.923013
"""
...
revision: str = '2802eb62c668'
down_revision: Union[str, None] = 'add_security_columns_20260902'
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('ticket_verification', sa.Column('send_attempts', sa.INTEGER(), server_default=sa.text('1'), autoincrement=False, nullable=False))
    op.add_column('ticket_verification', sa.Column('verified_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True))
    op.add_column('ticket_verification', sa.Column('resend_count', sa.INTEGER(), server_default=sa.text('0'), autoincrement=False, nullable=False))
    op.add_column('ticket_verification', sa.Column('last_sent_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True))
    op.add_column('knowledge_injection_job', sa.Column('attempts', sa.INTEGER(), server_default=sa.text('0'), autoincrement=False, nullable=False))
